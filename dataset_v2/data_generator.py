...
def load_files(sql_path,table_path,out_path,flag):
    ...
    for example in data_test:
        ...
        if flag == "undirected":
            ...
            g_adj = add_edge_directed("0",str(node_cou),g_adj)
        ...
        if agg_type != "":
            ...
            if flag == "undirected":
                ...
                g_adj = add_edge_directed(str(node_cou),"1",g_adj)
        ...
        for cond in conds:
            ...
            g_adj[str(node_cou)] = []
            if flag == "undirected":
                g_adj = add_edge(str(and_id),str(node_cou),g_adj)
            else:
                g_adj = add_edge_directed(str(and_id),str(node_cou),g_adj)
            

            # cond_val node
            if str(cond_val).lower().strip() not in cond_val_set:
                cond_val_cou += 1
                node_cou += 1
                cond_val_set[str(cond_val).lower().strip()] = (cond_val_cou,node_cou)
                g_ids[str(node_cou)] = node_cou
                g_id_feature[str(node_cou)] = cond_op+"val"+str(cond_val_cou)
                g_adj[str(node_cou)] = []

            cond_val_idx,cond_node_idx = cond_val_set[str(cond_val).lower().strip()]

            
            if flag == "undirected":
                g_adj = add_edge(str(node_cou-1),str(cond_node_idx),g_adj)
            else:
                g_adj = add_edge_directed(str(node_cou-1),str(cond_node_idx),g_adj)
